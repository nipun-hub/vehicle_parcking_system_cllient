...
import AddDiscountAlert from './Alert/AddDiscountAlert';
import DeleteDiscount from './Alert/DeleteDiscount';
import DiscountHead from './DiscountHead';
import { getDiscount } from '../../../services/Common/CommonService';

const columns = ["Name", "Percentage", 'Start date', 'End Date', 'Status', 'Action'];
...
export default function Discount() {
    ...
    return (
        <>
            {(addItemOpen || updateOpen) && <AddDiscountAlert open={updateOpen ? updateOpen : addItemOpen} close={updateOpen ? closeUpdateAlert : closeAddItemAlert} data={updateOpen ? selectedItemData : null} />}
            <DeleteDiscount open={deleteOpen} close={closeDeleteAlert} data={selectedItemData} />
            <View open={viewOpen} close={() => serViewOpen(false)} data={selectedItemData} />

            <DiscountHead setOpen={setAddItemOpen} />
            ...
            <Paper sx={{ overflow: 'hidden' }} className='mt-10 max-w-full me-0.5'>
                <TableContainer >
                    <Table stickyHeader aria-label="sticky table">
                        ...
                        <TableBody>
                            {discountList && Array.isArray(discountList) && discountList
                                ...
                                .map((row, rowId) => {
                                    return (
                                        <TableRow hover role="checkbox" tabIndex={-1} key={rowId}>
                                            ...
                                            <TableCell className='border-s-2 border-gray-200 '>{row.percentage} %</TableCell>
                                            <TableCell className='border-s-2 border-gray-200 '>{row.startDate}</TableCell>
